...
const schedule_update_following_by_id_old = async (victim) => {
    try {
...
const get_and_update_following_by_id = async (victim, current_followings) => {
    ...
    try {
        while (cursor.startsWith('0|') === false) {
            ...
            response_followings = parse_following_from_api_entries_old(victim.app_username, victim.victim_id, response_followings.entries, current_followings)
...
const parse_following_from_api_entries_old = (app_username, victim_id, response_followings, current_followings) => {
    const followings = {}
    const cursor_top = response_followings[response_followings.length - 1].content.value
    response_followings.pop()
    const cursor_bottom = response_followings[response_followings.length - 1].content.value
...
module.exports = {
    ...
    schedule_update_following_by_id_old,
}
