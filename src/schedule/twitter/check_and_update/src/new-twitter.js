const { database, twitter, entity } = require('/opt/nodejs')
const { send_message } = require('./telegram')
class TwitterSchedulerClient {
    constructor() {
        this.processing_victims = []
        this.users = {}
    }
    ...
    async get_followings_by_id(victim) {
        try {
            const [
                current_followings,
                response_followings,
    ...
    async get_response_followings_by_id(victim) {
        let cursor = '-1'
        let followings = {}
        try {
    ...
    parse_followings_from_api_entries(app_username, victim_id, response_followings) {
        const followings = {}
