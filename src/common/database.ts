...
import { User, Victim, Following } from './entity';
...
export const get_user_by_username = async (app_username: string): Promise<User> => {
    ...
    const result = await client.get({
        ...
        Key: {
            ...
            SK: `USER@${app_username}`
        }
    }).promise()
    return User.fromORM(result.Item)
}

export const put_entity = async (entity) => {
    try {
        await client.put({
            TableName: TABLE_NAME,
            Item: entity.toORM(),
...
export const batch_update_following = async (put_entities, delete_entities) => {
    try {
